...
def apicUpgrade(specDict, cookie):
    ...
    for upgradeCheck in range(0, 90):
        print("Iteration: " + str(upgradeCheck))
        #if bakeTimer >= 30:
        try:
            cookie = bakeCookies(specDict)
            bakeTimer = 0
            try:
                ...
                #pprint.pprint(upgradeJson)
                try:
                    upgradeResult = upgradeJson["imdata"][0]["maintUpgJob"]["attributes"]["upgradeStatus"]
                    upgradePercent = upgradeJson["imdata"][0]["maintUpgJob"]["attributes"]["instlProgPct"]
                ...
                print("Unable to get status of upgrade at:\t" + upgradeCheckURL)
                print("http response:\t" + str(upgradeStatus))
                pprint.pprint(upgradeJson)
                upgradeCheck += 1
                time.sleep(60)
        except:
            print("Unable to obtain auth token")
            upgradeCheck += 1
            time.sleep(60)
